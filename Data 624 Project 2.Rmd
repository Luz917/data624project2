...
Two files are provided for this project. The file `StudentData.csv` contains the data used to build the predictive models for <i>PH</i>. The file `StudentEvaluation.csv` contains the data used to evaluate the selected model. The `PH` column in this file has no data.
...
The `student_data` data frame is copied before we start modifying the data frame by dropping variables and imputing missing values. 
...
PLS_model
```
Below is the plot for PLS model's RMSE and number of components. 
